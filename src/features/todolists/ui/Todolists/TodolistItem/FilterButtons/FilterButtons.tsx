...
export const FilterButtons = ({id, filter}:FilterButtonsType) => {
  ...
  return (
    ...
    <ButtonClick title={"Active"} changeFilter={() => changeFilterHandler("Active")} filter={filter} color={'secondary'} />
    <ButtonClick title={"Completed"} changeFilter={() => changeFilterHandler("Completed")} filter={filter} color={'success'} />
  </Box>)
}